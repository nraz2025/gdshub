    ...
    <div style={{fontFamily:'Inter, system-ui, sans-serif', background:'#f1f5f9', minHeight:'100vh'}}>
      <div style={{padding:'20px 28px',marginBottom:'0'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'12px'}}>
          <div>
            <h1 style={{fontSize:'30px',fontWeight:700,color:'#1e293b',margin:0,letterSpacing:'-0.02em'}}>Travelport Users</h1>
            ...
            filtered.map((row,i)=>{
              ...
              return (
                <div key={row.id} style={{display:'grid',gridTemplateColumns:'minmax(130px,1.3fr) minmax(110px,1.1fr) minmax(200px,2fr) minmax(90px,0.9fr) minmax(90px,0.9fr) minmax(80px,0.8fr) minmax(110px,1.1fr) minmax(170px,170px)',minWidth:'980px',borderBottom:i<filtered.length-1?'1px solid #e2e8f0':'none',transition:'background 0.3s cubic-bezier(0.4,0,0.2,1)'}}
                  ...
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',borderRight:'1px solid #f1f5f9'}}><span style={{fontFamily:'monospace',fontSize:'16px',color:'#64748b',letterSpacing:'0.05em'}}>{row.sign_on_id??'-'}</span></div>
                  <div style={{padding:'14px 16px',display:'flex',flexDirection:'column',justifyContent:'center',borderRight:'1px solid #f1f5f9'}}>{u?<><div style={{fontSize:'16px',fontWeight:600,color:'#1e293b'}}>{u.first_name} {u.last_name}</div><div style={{fontSize:'15px',color:'#94a3b8'}}>{u.email_address}</div></>:<span style={{fontSize:'14px',color:'#94a3b8'}}>-</span>}</div>
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',borderRight:'1px solid #f1f5f9'}}><span style={{fontFamily:'monospace',fontSize:'16px',color:'#64748b',textTransform:'uppercase',letterSpacing:'0.05em'}}>{row.cid??'-'}</span></div>
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',borderRight:'1px solid #f1f5f9'}}><span style={{fontFamily:'monospace',fontSize:'16px',color:'#64748b',textTransform:'uppercase',letterSpacing:'0.05em'}}>{row.gtid??'-'}</span></div>
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',borderRight:'1px solid #f1f5f9'}}>
                    <span style={{fontSize:'16px',fontWeight:600,padding:'4px 10px',borderRadius:'9999px',background:row.ota?'#f0fdf4':'#f1f5f9',color:row.ota?'#16a34a':'#94a3b8',border:`1px solid ${row.ota?'#bbf7d0':'#e2e8f0'}`}}>{row.ota?'Yes':'No'}</span>
                  </div>
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',borderRight:'1px solid #f1f5f9'}}><span style={{display:'inline-flex',alignItems:'center',gap:'6px',fontSize:'16px',fontWeight:600,padding:'6px 14px',borderRadius:'9999px',background:sval==='active'?'#f0fdf4':ss.bg,color:sval==='active'?'#16a34a':ss.color,border:`1px solid ${sval==='active'?'#bbf7d0':ss.border}`,textTransform:'capitalize'}}><span style={{width:'6px',height:'6px',borderRadius:'50%',background:sval==='active'?'#22c55e':ss.color,flexShrink:0}}/>{sval}</span></div>
