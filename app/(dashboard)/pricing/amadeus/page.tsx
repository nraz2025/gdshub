...
const D = {
  ...
  accent: '#a371f7', accentSoft: 'rgba(163,113,247,0.10)', titleColor: '#d3bbfc',
...
export default function PricingAmadeusPage() {
      ...
      <div style={{padding:'32px 28px 40px'}}>
        ...
        <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:'16px', marginBottom:'22px'}}>
          <div>
            <h1 style={{fontFamily:"'Space Grotesk', sans-serif", fontSize:'26px', fontWeight:700, letterSpacing:'-0.5px', color:D.fg, margin:0}}>
              <span style={{display:'inline-flex', alignItems:'center', fontSize:'15px', fontWeight:700, color:D.accent, background:D.accentSoft, border:`1px solid ${D.accent}40`, borderRadius:'8px', padding:'5px 12px', marginRight:'10px', verticalAlign:'middle'}}>Amadeus</span>
              Pricing
            </h1>
        ...
        {loading ? (
          ...
          <div style={{background:D.card, border:`1px solid ${D.border}`, borderRadius:'10px', padding:'48px', textAlign:'center', color:D.fgDim, fontSize:'15px'}}>
            {records.length === 0 ? 'No pricing items yet. Add the first Amadeus pricing item to get started.' : 'No pricing items match your search or filter.'}
          </div>
        ) : (
                  ...
                  {productNames.map((product, pi) => {
                    ...
                    return tiers.map((r, i) => (
                      <tr key={r.id} style={{borderBottom: (isLastProduct && i === tiers.length - 1) ? 'none' : `1px solid ${D.border}`}}
                        ...
                        {i === 0 && (
                          <td rowSpan={tiers.length} style={{padding:'12px 18px', verticalAlign:'top', borderRight:`1px solid ${D.border}`, borderLeft:`3px solid ${D.accent}`}}>
