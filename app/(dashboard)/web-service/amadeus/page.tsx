...
interface WebServiceRow {
  ...
  agent_id: string
  wbs_user_id: string | null
  password: string
  password_email_link: string | null
  notes: string | null
  source: string | null
...
const EMPTY = { pcc: '', company_name: '', agent_id: '', wbs_user_id: '', password: '', password_email_link: '', notes: '', source: '' }

const GDS_NAME = 'Amadeus'

// Dark theme (accent matches Amadeus's color elsewhere in the app)
const D = {
...
export default function WebServiceAmadeusPage() {
  ...
  function openAdd() { setEditing(null); setForm(EMPTY); setError(''); resetAttachmentState(); setModalOpen(true) }
  function openEdit(row: WebServiceRow) {
    setEditing(row)
    setForm({
      pcc: row.pcc, company_name: row.company_name ?? '', agent_id: row.agent_id, wbs_user_id: row.wbs_user_id ?? '',
      password: row.password, password_email_link: row.password_email_link ?? '', notes: row.notes ?? '', source: row.source ?? '',
    })
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      agent_id: form.agent_id.trim(), wbs_user_id: form.wbs_user_id.trim() || null,
      password: form.password.trim(), password_email_link: form.password_email_link.trim() || null,
  ...
  function handleExport() {
    const data = filtered.map(r => ({
      'PCC / OID': r.pcc,
      'Company Name/Apps Name': r.company_name ?? '',
      'WSAP': r.agent_id,
      'WBS User ID': r.wbs_user_id ?? '',
      'Password': r.password,
      'Password Email Link': r.password_email_link ?? '',
      'Notes': r.notes ?? '',
      'Source': r.source ?? '',
    }))
  ...
  function handleDownloadTemplate() {
    const ws = XLSX.utils.json_to_sheet([
      { 'PCC / OID': 'B7Y8', 'Company Name/Apps Name': 'Forecepts New System', 'WSAP': '935028', 'WBS User ID': '', 'Password': 'WS258022', 'Password Email Link': '', 'Notes': '', 'Source': 'Automation Hub' },
    ])
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, `${GDS_NAME} Web Service`)
  ...
  function handleFilePick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      ...
      const parsed: ImportRow[] = raw.map(r => {
        ...
        const agent_id = String(r['WSAP'] ?? r['Agent ID / WS'] ?? r['agent_id'] ?? '').trim()
        const wbs_user_id = String(r['WBS User ID'] ?? r['wbs_user_id'] ?? '').trim()
        const password = String(r['Password'] ?? r['password'] ?? '').trim()
        const password_email_link = String(r['Password Email Link'] ?? r['password_email_link'] ?? '').trim()
        const company_name = String(r['Company Name/Apps Name'] ?? r['Company'] ?? r['company_name'] ?? '').trim()
        const notes = String(r['Notes'] ?? r['notes'] ?? '').trim()
        const source = String(r['Source'] ?? r['source'] ?? '').trim()
        return { pcc, company_name, agent_id, wbs_user_id, password, password_email_link, notes, source, valid: true, error: undefined }
      })
  ...
  async function handleImportConfirm() {
    ...
    for (const r of validRows) {
      const { error: e } = await supabase.from('web_service').insert({
        ...
        agent_id: r.agent_id, wbs_user_id: r.wbs_user_id || null,
        password: r.password, password_email_link: r.password_email_link || null,
      ...
      <div style={{padding:'32px 28px 40px'}}>
        ...
        {loading ? (
          ...
          <div style={{background:D.card, border:`1px solid ${D.border}`, borderRadius:'10px', overflow:'auto', maxHeight:'75vh'}}>
            <table style={{width:'100%', borderCollapse:'collapse', minWidth:'900px'}}>
              <thead>
                <tr>
                  {['PCC / OID', 'Company Name/Apps Name', 'WSAP', 'WBS User ID', 'Password', 'URL', 'Notes', 'Attachment', 'Actions'].map((h, i, arr) => (
                ...
                {filtered.map((row, i) => {
                  ...
                  return (
                    <tr key={row.id} style={{borderBottom: i < filtered.length - 1 ? `1px solid ${D.border}` : 'none', transition:'background 0.15s'}}
                      ...
                      <td style={{padding:'12px 16px', fontFamily:'monospace', fontSize:'13px', color:D.fgMuted}}>{row.wbs_user_id ?? <span style={{color:D.fgDim}}>—</span>}</td>
                      ...
                      <td style={{padding:'12px 16px'}}>
                        {row.password_email_link ? (
                          <a href={row.password_email_link} target="_blank" rel="noopener noreferrer" title={row.password_email_link}
                            style={{display:'inline-flex', alignItems:'center', gap:'6px', color:D.accent, textDecoration:'none', fontSize:'13px', fontWeight:600}}
                            onMouseOver={e => (e.currentTarget.style.textDecoration = 'underline')}
                            onMouseOut={e => (e.currentTarget.style.textDecoration = 'none')}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0}}><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                            Open
                          </a>
        ...
        {modalOpen && (
          ...
          <div style={{width:'100%', maxWidth:'560px', maxHeight:'90vh', overflowY:'auto', background:D.card, border:`1px solid ${D.accent}`, borderRadius:'14px', padding:'22px', boxShadow:`0 20px 60px rgba(0,0,0,0.4), 0 0 0 3px ${D.accentSoft}`}}>
            ...
            <div style={{marginBottom:'14px'}}>
              <label style={lblDark}>Company Name/Apps Name</label>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px'}}>
              <div>
                <label style={lblDark}>WSAP</label>
              ...
              <div>
                <label style={lblDark}>WBS User ID</label>
                <input type="text" value={form.wbs_user_id} onChange={e => setForm(f => ({ ...f, wbs_user_id: e.target.value }))} placeholder="e.g. 200123" style={{...inpDark(), fontFamily:'monospace'}} />
              </div>
            </div>

            <div style={{marginBottom:'14px'}}>
              <label style={lblDark}>Auto-generate Password</label>
              <div style={{display:'flex', gap:'8px'}}>
                <input type="text" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Click Generate, or type your own" style={{...inpDark(), fontFamily:'monospace'}} />
              ...
              <p style={{fontSize:'11px', color:D.fgDim, marginTop:'4px'}}>Blank until you click Generate (WS + 6-digit format), or type your own.</p>
            </div>

            <div style={{marginBottom:'14px'}}>
              <label style={lblDark}>URL</label>
              <input type="url" value={form.password_email_link} onChange={e => setForm(f => ({ ...f, password_email_link: e.target.value }))} placeholder="Paste link to the email containing the password" style={inpDark()} />
            </div>
          ...
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(4px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px'}}
            ...
            <div style={{width:'100%', maxWidth:'720px', maxHeight:'90vh', overflowY:'auto', background:D.card, border:`1px solid ${D.accent}`, borderRadius:'14px', padding:'22px', boxShadow:`0 20px 60px rgba(0,0,0,0.4), 0 0 0 3px ${D.accentSoft}`}}>
              ...
              <p style={{fontSize:'12px', color:D.fgDim, marginBottom:'16px'}}>
                {importFileName} — columns expected: PCC / OID, Company Name/Apps Name, WSAP, WBS User ID, Password, Password Email Link, Notes, Source.{' '}
              ...
              {importResult ? (
                ...
                <div style={{border:`1px solid ${D.border}`, borderRadius:'10px', overflow:'auto', maxHeight:'340px', marginBottom:'16px'}}>
                  <table style={{width:'100%', borderCollapse:'collapse', fontSize:'13px'}}>
                    <thead>
                      <tr>
                        {['PCC / OID', 'Company Name/Apps Name', 'WSAP', 'WBS User ID', 'Password', 'Notes', 'Status'].map(h => (
                    ...
                    <tbody>
                      {importRows.map((r, i) => (
                        <tr key={i} style={{borderBottom:`1px solid ${D.border}`, background: r.valid ? 'transparent' : D.dangerSoft}}>
                          ...
                          <td style={{padding:'8px 12px', fontFamily:'monospace', color:D.fgMuted}}>{r.wbs_user_id || '—'}</td>
