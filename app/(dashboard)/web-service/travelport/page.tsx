...
const EMPTY = { pcc: '', company_name: '', agent_id: '', password: '', password_email_link: '', notes: '', source: '' }
...
export default function WebServiceTravelportPage() {
  ...
  function openEdit(row: WebServiceRow) {
    setEditing(row)
    setForm({
      pcc: row.pcc, company_name: row.company_name ?? '', agent_id: row.agent_id,
      password: row.password, password_email_link: row.password_email_link ?? '',
  ...
  async function handleSave() {
    ...
    if (!form.agent_id.trim()) { setError('Username is required.'); return }
    ...
    const payload = {
      ...
      password_email_link: form.password_email_link.trim() || null,
  ...
  async function handleDelete(row: WebServiceRow) {
    if (!confirm(`Delete web service for PCC ${row.pcc} (Username ${row.agent_id})? This cannot be undone.`)) return
  ...
  function handleExport() {
    const data = filtered.map(r => ({
      'PCC / OID': r.pcc,
      'Branch Code': r.company_name ?? '',
      'Username': r.agent_id,
      'Password': r.password,
      'URL': r.password_email_link ?? '',
      'Notes': r.notes ?? '',
      'Source': r.source ?? '',
    }))
  ...
  function handleDownloadTemplate() {
    const ws = XLSX.utils.json_to_sheet([
      { 'PCC / OID': 'B7Y8', 'Branch Code': 'Forecepts New System', 'Username': '935028', 'Password': 'WS258022', 'URL': '', 'Notes': '', 'Source': 'Automation Hub' },
    ])
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, `${GDS_NAME} Web Service`)
  ...
  function handleFilePick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (evt) => {
      ...
      const parsed: ImportRow[] = raw.map(r => {
        ...
        const agent_id = String(r['Username'] ?? r['Agent ID / WS'] ?? r['agent_id'] ?? '').trim()
        const password = String(r['Password'] ?? r['password'] ?? '').trim()
        const password_email_link = String(r['URL'] ?? r['password_email_link'] ?? '').trim()
        const company_name = String(r['Branch Code'] ?? r['Company'] ?? r['company_name'] ?? '').trim()
        const notes = String(r['Notes'] ?? r['notes'] ?? '').trim()
        const source = String(r['Source'] ?? r['source'] ?? '').trim()
        let error: string | undefined
        if (!pcc) error = 'Missing PCC / OID'
        else if (!agent_id) error = 'Missing Username'
        else if (!password) error = 'Missing Password'
        return { pcc, company_name, agent_id, password, password_email_link, notes, source, valid: !error, error }
      })
  ...
  async function handleImportConfirm() {
    ...
    for (const r of validRows) {
      const { error: e } = await supabase.from('web_service').insert({
        ...
        agent_id: r.agent_id, password: r.password, password_email_link: r.password_email_link || null,
      ...
      <div style={{padding:'32px 28px 40px'}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'12px', marginBottom:'18px', flexWrap:'wrap'}}>
          <div style={{position:'relative', flex:'1 1 240px'}}>
            ...
            <input type="text" placeholder="Search PCC, Username, or Branch Code..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        {loading ? (
          ...
          <div style={{background:D.card, border:`1px solid ${D.border}`, borderRadius:'10px', overflow:'auto', maxHeight:'75vh'}}>
            <table style={{width:'100%', borderCollapse:'collapse', minWidth:'900px'}}>
              <thead>
                <tr>
                  {['PCC / OID', 'Branch Code', 'Username', 'Password', 'URL', 'Notes', 'Attachment', 'Actions'].map((h, i, arr) => (
        ...
        {modalOpen && (
          ...
          <div style={{width:'100%', maxWidth:'560px', maxHeight:'90vh', overflowY:'auto', background:D.card, border:`1px solid ${D.accent}`, borderRadius:'14px', padding:'22px', boxShadow:`0 20px 60px rgba(0,0,0,0.4), 0 0 0 3px ${D.accentSoft}`}}>
            ...
            <div style={{marginBottom:'14px'}}>
              <label style={lblDark}>Branch Code</label>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'4px'}}>
              <div>
                <label style={lblDark}>Username <span style={{color:D.danger}}>*</span></label>
              ...
              <div>
                ...
                <div style={{display:'flex', gap:'6px'}}>
                  <input type="text" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Click Generate" style={{...inpDark(), fontFamily:'monospace'}} />
                  <button type="button" onClick={() => setForm(f => ({ ...f, password: generateWsPassword() }))} title="Generate"
                    style={{display:'flex', alignItems:'center', justifyContent:'center', padding:'0 10px', background:D.accentSoft, border:`1px solid ${D.accent}`, borderRadius:'8px', color:D.accent, cursor:'pointer', flexShrink:0}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
                  </button>
                </div>
              </div>
            </div>
            <p style={{fontSize:'11px', color:D.fgDim, marginTop:'0', marginBottom:'14px'}}>Password blank until you click Generate (WS + 6-digit format), or type your own.</p>
          ...
          <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(4px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px'}}
            ...
            <div style={{width:'100%', maxWidth:'720px', maxHeight:'90vh', overflowY:'auto', background:D.card, border:`1px solid ${D.accent}`, borderRadius:'14px', padding:'22px', boxShadow:`0 20px 60px rgba(0,0,0,0.4), 0 0 0 3px ${D.accentSoft}`}}>
              ...
              <p style={{fontSize:'12px', color:D.fgDim, marginBottom:'16px'}}>
                {importFileName} — columns expected: PCC / OID, Branch Code, Username, Password, URL, Notes, Source.{' '}
              ...
              {importResult ? (
                ...
                <div style={{border:`1px solid ${D.border}`, borderRadius:'10px', overflow:'auto', maxHeight:'340px', marginBottom:'16px'}}>
                  <table style={{width:'100%', borderCollapse:'collapse', fontSize:'13px'}}>
                    <thead>
                      <tr>
                        {['PCC / OID', 'Branch Code', 'Username', 'Password', 'URL', 'Notes', 'Status'].map(h => (
                    ...
                    <tbody>
                      {importRows.map((r, i) => (
                        <tr key={i} style={{borderBottom:`1px solid ${D.border}`, background: r.valid ? 'transparent' : D.dangerSoft}}>
                          ...
                          <td style={{padding:'8px 12px', color:D.fgMuted}}>{r.password_email_link || '—'}</td>
