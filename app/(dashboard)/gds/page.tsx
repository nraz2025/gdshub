...
export default function GDSPage() {
  ...
  async function handleSave() {
    ...
    setError('')
    if (editing) {
      const { error } = await supabase.from('gds').update({ name: form.name }).eq('id', editing.id)
      if (error) { setError(error.message); setSaving(false); return }
    } else {
      const { error } = await supabase.from('gds').insert({ name: form.name })
      if (error) { setError(error.message); setSaving(false); return }
    }
    setSaving(false)
    setModalOpen(false)
  ...
  async function handleDelete() {
    if (!editing) return
    setSaving(true)
    const { error } = await supabase.from('gds').delete().eq('id', editing.id)
    if (error) { setError(error.message); setSaving(false); return }
    setSaving(false)
    setDeleteOpen(false)
    fetchAll()
  ...
  const GDS_COLORS: Record<GDSName, string> = {
  ...
  const columns = [
    {
      ...
      label: 'GDS Name',
      render: (row: GDS) => (
        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${GDS_COLORS[row.name] ?? 'bg-slate-100 text-slate-600'}`}>
          {row.name}
        </span>
      )
    },
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={editing ? 'Edit GDS' : 'Add GDS'}
        size="sm"
      >
