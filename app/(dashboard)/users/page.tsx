    ...
    <div style={{fontFamily:"Inter, system-ui, sans-serif", background:T.surface, minHeight:'100vh'}}>
          ...
          {isAdmin && (
            <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
              <button onClick={handleExport} disabled={filtered.length === 0}
                style={{display:'flex', alignItems:'center', gap:'6px', padding:'8px 14px', background:'white', border:`1px solid ${T.border}`, borderRadius:T.radius, fontSize:'13px', fontWeight:500, color:T.textMid, cursor:'pointer', opacity:filtered.length===0?0.4:1}}>
              ...
              <button onClick={() => fileInputRef.current?.click()}
                style={{display:'flex', alignItems:'center', gap:'6px', padding:'8px 14px', background:'white', border:`1px solid ${T.border}`, borderRadius:T.radius, fontSize:'13px', fontWeight:500, color:T.textMid, cursor:'pointer'}}>
      ...
      <div style={{padding:'0 28px 28px'}}>
        ...
        <div style={{background:'white', border:`1px solid ${T.border}`, borderRadius:T.radius, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
          {/* Header */}
          <div style={{display:'grid', gridTemplateColumns:'2.5fr 1fr 1fr 1fr 140px', background:T.surfaceAlt, borderBottom:`2px solid ${T.border}`}}>
                  ...
                  <div style={{padding:'14px 16px', display:'flex', alignItems:'center', justifyContent:'flex-end', gap:'6px'}}>
                    {isAdmin && (<>
                      <button onClick={() => openEdit(u)}
                        style={{padding:'5px 14px', fontSize:'12px', fontWeight:600, color:T.textMid, background:'white', border:`1px solid ${T.border}`, borderRadius:T.radius, cursor:'pointer'}}>
                        Edit
                      </button>
                      <button onClick={() => openDelete(u)}
                        style={{padding:'5px 14px', fontSize:'12px', fontWeight:600, color:T.danger, background:'white', border:'1px solid #fecaca', borderRadius:T.radius, cursor:'pointer'}}>
                        Delete
                      </button>
                    </>)}
                  </div>
        ...
        {/* ── Pagination ── */}
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:'14px', padding:'10px 16px', background:'white', border:`1px solid ${T.border}`, borderRadius:T.radius, flexWrap:'wrap', gap:'10px'}}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
            ...
            <div style={{display:'flex', alignItems:'center', gap:'6px'}}>
              ...
              <select value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setCurrentPage(1) }}
                style={{padding:'3px 8px', fontSize:'12px', border:`1px solid ${T.border}`, borderRadius:T.radius, background:'white', color:T.textMid, cursor:'pointer', outline:'none'}}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit User' : 'Add User'}>
        <div className="space-y-4">
          ...
          <div style={{display:'flex', gap:'10px', paddingTop:'4px'}}>
            <button onClick={() => setModalOpen(false)} style={{flex:1, padding:'9px', fontSize:'13px', fontWeight:500, border:`1px solid ${T.border}`, borderRadius:T.radius, background:'white', color:T.textMid, cursor:'pointer'}}>Cancel</button>
      ...
      <Modal open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete User" size="sm">
        <div className="space-y-4">
          ...
          <div style={{display:'flex', gap:'10px'}}>
            <button onClick={() => setDeleteOpen(false)} style={{flex:1, padding:'9px', fontSize:'13px', fontWeight:500, border:`1px solid ${T.border}`, borderRadius:T.radius, background:'white', color:T.textMid, cursor:'pointer'}}>Cancel</button>
      ...
      <Modal open={importOpen} onClose={closeImport} title="Import Users" size="lg">
        <div className="space-y-4">
          ...
          <div style={{display:'flex', gap:'10px', paddingTop:'4px'}}>
            <button onClick={closeImport} style={{flex:1, padding:'9px', fontSize:'13px', fontWeight:500, border:`1px solid ${T.border}`, borderRadius:T.radius, background:'white', color:T.textMid, cursor:'pointer'}}>{importResult ? 'Close' : 'Cancel'}</button>
